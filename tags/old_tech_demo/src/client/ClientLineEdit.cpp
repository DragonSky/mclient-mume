...
void ClientLineEdit::toggleEchoMode() {
...
bool ClientLineEdit::emitFakeKey(QKeyEvent *ke) {
...
bool ClientLineEdit::event(QEvent *event)
{
...
void ClientLineEdit::inputMoveTo(const int& new_pos) {
  //setSelection(new_pos, cursorPosition() - new_pos);
  setCursorPosition(new_pos);
}

void ClientLineEdit::inputInsertText(const QString& input){
  setSelection(cursorPosition(), input.length());
  insert(input);
}

void ClientLineEdit::inputDeleteChars(const int& num) {
  setSelection(cursorPosition(), num);
  del();
}

void ClientLineEdit::inputClear() {
  clear();
}
