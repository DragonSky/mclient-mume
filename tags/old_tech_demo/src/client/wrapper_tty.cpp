...
void tty_puts(const char *s) { 
  QString result = s;
  if (!result.isEmpty()) PowwowWrapper::self()->writeToStdout(result);
}
void tty_putc(unsigned char c) { PowwowWrapper::self()->writeToStdout(QString(c)); }
void tty_gets(char* s, int size) { PRINTF("<-- tty_gets not implemented\n"); }
int tty_read(char *buf, int cnt) { PRINTF("<-- tty_read not implemented\n"); }//return PowwowWrapper::self()->wrapper_stdin.read(buf, cnt); }

void tty_gotoxy_opt(int fromcol, int fromline, int tocol, int toline) {
  PowwowWrapper::self()->emitMoveCursor(fromcol, fromline, tocol, toline);
}

void tty_gotoxy(int col, int line) {
  qDebug("implement me?");
  //PowwowWrapper::self()->emitMoveCursor(-1, -1, col, line);
}
void wrapper_input_set(char *str) { PowwowWrapper::self()->emitInputSet(str); }

void input_overtype_follow(char c) {
  PowwowWrapper::self()->emitInputInsertText(QString(c));
  pos++;
}
void input_delete_nofollow_chars(int n) {
  qDebug("deleting %d chars", n);
  PowwowWrapper::self()->emitInputDeleteChars(n);
  pos -= n;
}
void input_moveto(int new_pos) {
  qDebug("moving cursor to %d", new_pos);
  PowwowWrapper::self()->emitInputMoveTo(new_pos);
  pos = new_pos;
}
void input_insert_follow_chars(char *str, int n) {
  PowwowWrapper::self()->emitInputInsertText(QString(str).left(n));
  pos += n;
