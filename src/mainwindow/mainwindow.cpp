...
MainWindow::MainWindow() {
  ...
  vbox->addWidget(_clientManager->getTextView());
  vbox->addWidget(_clientManager->getInputBar());
  ...
  m_dockDialogGroup = new QDockWidget(tr("Group"), this);
  ...
  m_dockDialogGroup->setWidget(_groupManager->getGroup());
...
void MainWindow::start(int argc, char **argv) {
  ...
  ClientManager::self()->getInputBar()->setFocus();
}
...
void MainWindow::editObjects() {
  if (!objectEditor) {
    objectEditor = new ObjectEditor(Wrapper::self(), this);
  }
  objectEditor->show();
...
void MainWindow::profileSelected() {
  ...
  Wrapper::self()->loadProfile(profile);
}
