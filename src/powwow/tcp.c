...
#else
    wrapper_tcp_connect(host, port, initstring, i);
}

/*
 * This is the slot that Qt will connect to once the connection has been established.
 * This was done in order to prevent a blocking check whether the host existed or not
 */
void wrapper_tcp_connect_slot __P4 (char *,initstring, int,port, int,i, int,newtcp_fd)
{
    if (newtcp_fd < 0) {
        free(CONN_INDEX(i).host);
        free(CONN_INDEX(i).id);
