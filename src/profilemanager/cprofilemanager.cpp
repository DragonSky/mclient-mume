...
#include "configuration.h" //#include "TODO cGlobalSettings.h"
...
{
  QString path = Config().profilePath; // TODO
...
//  return QString();   // not found
  return Config().profilePath + "/"; //TODO cGlobalSettings::self()->profilePath() + "/";
}
...
QString cProfileManager::newProfile (const QString &name)
{
  ...
  QDir mainDir (Config().profilePath); //TODO cGlobalSettings::self()->profilePath());
  int num = 0;
  QString dirname;
  bool found = false;
...
void cProfileManager::load ()
{
  ...
  QString path = Config().profilePath; // TODO cGlobalSettings::self()->profilePath();
  QDir dir = QDir (path);
...
void cProfileManager::save ()
{
  ...
  if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text)) {
    qDebug() << "Unable to open profiles.xml for writing.";
    return;
  }
